...
const hasAllowedLength = (length, allowedLengths) =>
...
export const getPhoneValidationMessage = (phone = "", country = "Philippines") => {
  ...
  if (!hasAllowedLength(digits.length, allowedLengths)) {
...
export const limitPhoneNumberLength = (phone = "", country = "Philippines") => {
  const normalizedPhone = normalizePhoneNumber(phone);
  const startsWithPlus = normalizedPhone.startsWith("+");
  const rule = getPhoneRule(country);
  const maxDigits = Math.max(
    ...(startsWithPlus ? rule.internationalLengths : rule.localLengths)
  );
  const digits = normalizedPhone.replace(/\D/g, "").slice(0, maxDigits);
